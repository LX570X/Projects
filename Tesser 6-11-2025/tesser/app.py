...
import pytesseract
from flask import Flask, render_template, request, send_from_directory, redirect, send_file, jsonify, Response
...
def ocr_trim():
    ...
    try:
        osd_result = pytesseract.image_to_osd(bw)
        if "Script: Arabic" in osd_result:
            lang = "ara"
        else:
            lang = "eng"
    except:
...
def export_txt(filename):
    ...
    return Response(content, mimetype="text/plain", headers={"Content-Disposition": f"attachment; filename={filename}.txt"})
...
def export_docx_all():
    ...
    return send_file(doc_io, as_attachment=True, download_name="combined_output.docx", mimetype="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
